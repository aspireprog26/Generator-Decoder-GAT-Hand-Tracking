...
def calibrate():
    ...
    )  # Create an empty array to store 3D points of shape (num_corners, 3) with each row (X, Y, Z)
    objp[:, :2] = np.mgrid[0:8, 0:5].T.reshape(
        -1, 2
    )  # fill in the X and Y coordinates for each chessboard corner
    objp *= SQUARE_SIZE
    ...
    retL, K1, dist1, rvecs1, tvecs1 = cv2.calibrateCamera(
        obj_points, left_points, img_size, None, None
    )
    retR, K2, dist2, rvecs2, tvecs2 = cv2.calibrateCamera(
        obj_points, right_points, img_size, None, None
    )

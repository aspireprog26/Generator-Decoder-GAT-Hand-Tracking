...
class Video:
    def __init__(self, cam_index):
        ...
        self.standardize = Stats(device).standardize
        ...
        self.model = AnatomyModel(
            ...
            mano_root=r"c:\Users\Test\Documents\Hand-Tracking-2",
            generator=False,
    ...
    def getFrameMP(self):
        while self.running:
            if self.last_frame is not None:
                ...
                self.left_coords, left_handedness, left_handedness_score = (
                    self.pose_mp.get_keypoints(left)
                )
                self.right_coords, right_handedness, right_handedness_score = (
                    self.pose_mp.get_keypoints(right)
                )
    ...
    def start(self):
        takeFrame_thread = Thread(target=self.takeFrame, daemon=True)
        takeFrame_thread.start()
        mp_thread = Thread(target=self.getFrameMP, daemon=True)
        mp_thread.start()

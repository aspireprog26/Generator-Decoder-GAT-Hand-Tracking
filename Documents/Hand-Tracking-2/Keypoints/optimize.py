...
class Losses:
    ...
    def bone_dir_loss(self, stereo_aligned, target):
        ...
        for parent, child in self.bones:
            ...
            loss += ((bone_stereo - bone_target) ** 2).sum(
                dim=-1
            )  # Takes the Euclidean norm of each element and averages per bone across the batch
        return loss / len(self.bones)

    def bone_length_loss(self, stereo_aligned, target):
        loss = 0
        for parent, child in self.bones:
            length_stereo = torch.linalg.vector_norm(
                stereo_aligned[child] - stereo_aligned[parent], dim=-1
            )

            length_target = torch.linalg.vector_norm(
...
class Optimizer:
    def __init__(
        self, w1: float, w2: float, w3: float, w4: float, lr: float, num_steps: int
    ):
        self.w1 = w1
        self.w2 = w2
        self.w3 = w3
    ...
    def optimize(self, stereo_coords: np.ndarray, target: np.ndarray):
        ...
        stereo_aligned = torch.tensor(
            self.transform(stereo_coords), dtype=torch.float32
        )
        ...
        for _ in range(self.num_steps):
            ...
            loss = (
                (self.w1 * bone_dir_loss)
                + (self.w2 * bone_length_loss)
                + (self.w3 * bone_angle_loss)
                + (self.w4 * hand_loss)
            )
            loss.backward()
            optimizer.step()

...
def stereoTransform(pred_coords, coords_proj):
    ...
    pred_scale = torch.linalg.norm(
        pred_centered[:, 9] - pred_centered[:, 0], dim=-1, keepdim=True
    ).clamp_min(1e-8)
    pred_normalized = pred_centered / pred_scale.unsqueeze(-1)
...
def procrustesAlign(
    X,  # MANO predicted keypoints
    Y,  # Joint predictor keypoints
...
class Stats:
    def __init__(self, device):
        ...
        self.generator_mean = generator_stats[0].to(device, dtype=torch.float32)
        self.generator_std = generator_stats[1].to(device, dtype=torch.float32)

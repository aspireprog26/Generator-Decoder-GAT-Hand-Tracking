...
class Constrain:
    def __init__(self, img: np.ndarray, mp_conf: float):
        ...
        self.hands = mp_hands.Hands(
            static_image_mode=False, max_num_hands=1, min_detection_confidence=mp_conf
        )
...
class OptimizeHands:
    def __init__(
        self, stereo_coords: np.ndarray, left_hand: np.ndarray, right_hand: np.ndarray
    ):
        w1 = 0.2
        w2 = 0.4
        w3 = 0.3
    ...
    def optimize(self):
        try:
            ...
            if (
                (left_mp[0] == True and right_mp[0] == True)
                and (left_mp[1] is not None)
                and (right_mp[1] is not None)
            ):  # Checks if hand is detected by both views
                left_mp_kps = left_mp[1]
                right_mp_kps = right_mp[1]
                avg_mp_kps = (left_mp_kps + right_mp_kps) / 2

                self.left_stereo_optim = self.optimizer.optimize(
                    self.stereo_coords, left_mp_kps
                )
                self.right_stereo_optim = self.optimizer.optimize(
                    self.stereo_coords, right_mp_kps
                )
